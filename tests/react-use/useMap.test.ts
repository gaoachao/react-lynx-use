...
import { useMap } from "../../src/react-use";

const setUp = <T extends object>(initialMap?: T) =>
...
it("should get undefined for non-existing provided key", () => {
  const { result } = setUp<{ foo: string; a: number; nonExisting?: any }>({
    foo: "bar",
    a: 1,
  });
  const [, utils] = result.current;
...
it("should set new key-value pair", () => {
  const { result } = setUp<{ foo: string; a: number; newKey?: number }>({
    foo: "bar",
    a: 1,
  });
  const [, utils] = result.current;
...
it("should reset map to initial object provided", () => {
  const { result } = setUp<{ foo: string; a: number; z?: number }>({
    foo: "bar",
    a: 1,
  });
  const [, utils] = result.current;
